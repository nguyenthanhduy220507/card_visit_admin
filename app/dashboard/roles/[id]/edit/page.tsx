// app/dashboard/roles/[id]/edit/page.tsx
'use client';
...
export default function RoleEditPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setFormData({
        ...
        permissionIds: roleResponse.permissions?.map((p: { id: string }) => p.id) || [],
      });
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiService.updateRole(roleId, updateData);
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center py-12">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Vai trò có ID {roleId.substring(0, 8)}... không tồn tại hoặc đã bị xóa.
          </p>
        </div>
      </div>
    );
            ...
            <ol className="flex items-center space-x-2">
              ...
              <li>
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-gray-500">Chỉnh sửa vai trò</span>
                </div>
              </li>
            </ol>
          </nav>
